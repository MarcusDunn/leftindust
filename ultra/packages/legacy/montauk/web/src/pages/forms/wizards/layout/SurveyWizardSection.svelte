...
import SurveyWizardSelect from '../input/SurveyWizardSelect.svelte';
import { SurveyWizardActiveSectionIndex, SurveyWizardForm } from '../store/index';
        ...
        <Col width={!(field.dataType === DataType.MultiMuliSelect || field.dataType === DataType.SingleMuliSelect) ? '0' : '66'}>
          {#if Array.isArray(field.multiSelectPossibilities)}
            {#each field.multiSelectPossibilities as _, index}
              <SurveyWizardSelect {index} bind:options={field.multiSelectPossibilities} />
              <p />
            {/each}
            <AddUI
              title="Add Answer"
