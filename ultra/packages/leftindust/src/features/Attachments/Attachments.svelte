<script lang="ts">
  ...
  import { SpecificEngine } from '@/api/server/engines';
  ...
  export let attachments: Writable<Data[]> = writable([]);
...
'';
  ...
  export let reference: Data | undefined = undefined; 
  ...
  const getReferences = (newAttachments: Data[]) => {
  ...
  const submit = () => {
    const newAttachments = $temporaryAttachments.map((attachment) => <Data>JSON.parse(attachment));
