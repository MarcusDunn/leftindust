<script lang="ts">
  ...
  if (hour < 12) {
    message = $_('generics.goodMorning', { values: { name: $account.name?.firstName } });
  } else if (hour < 18) {
    message = $_('generics.goodAfternoon', { values: { name: $account.name?.firstName } });
  } else {
    message = $_('generics.goodEvening', { values: { name: $account.name?.firstName } });
  }
