...
internal class ReadEventServiceImplUnitTest {
    ...
    internal fun `check getByPatientId returns a patient's event when patient exists`() = runTest {
        ...
        val readEventServiceImpl = ReadEventServiceImpl(eventRepository, readPatientService, readVisitService, doctorRepository)
    ...
    @Test
    internal fun `check getByDoctorId returns null when no matching doctor exists corresponding to any events`() = runTest {
        val someNonExistentUUid = UUID.fromString("235b4875-92d4-4553-8852-eb8f4b3a887d")
