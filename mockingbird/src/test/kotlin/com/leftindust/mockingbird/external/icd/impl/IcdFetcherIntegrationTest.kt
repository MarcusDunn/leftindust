...
internal class IcdFetcherIntegrationTest(@Autowired private val icdFetcher: IcdFetcher) : NoAuthIntegrationTest() {
    ...
    fun `check getDetails success`(): Unit = runBlocking {
        val result = icdFetcher.getDetails(GraphQLFoundationIcdCode("http://id.who.int/icd/release/11/2020-09/mms/236924369/unspecified"))
        assertEquals("Human immunodeficiency virus disease associated with tuberculosis", result.title?.value)
    }
    ...
    fun `check linearizationEntity success`(): Unit = runBlocking {
        val result = icdFetcher.linearizationEntity(GraphQLFoundationIcdCode("http://id.who.int/icd/release/11/2020-09/mms/236924369/unspecified"))
